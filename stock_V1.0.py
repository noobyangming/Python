...
# df=pd.read_excel('1026.xlsx','Sheet1')
...
# #  df是已有的Dataframe类型数据
# df.to_sql('s_data', con=engine, index=False, if_exists='append')
...
#                 if i == (drop_times-1):
#                     sample_num += 1
#                     # print(df_stock.涨跌幅[n+i],df_stock.日期[n+i])
...
# # print((df_output))
# book = load_workbook('output1026.xlsx')
# writer = pd.ExcelWriter('output1026.xlsx', engine='openpyxl')
# writer.book = book
...
for t in t_list:
    ...
    df_tstock = df_tstock.sort_values(by="日期",ascending=True)
