...
drop_times = 4
df_output = pd.DataFrame(columns = ["简称", "下跌4次后上涨", "下跌4次", "比率"])
for sn in sn_list:
    ...
    df_output = df_output.append(pd.DataFrame({"简称":[sn], "下跌4次后上涨":[int(sample_num)], "下跌4次":[bingo_num], "比率":[bingo_num/sample_num]}),ignore_index=True)
print(df_output)
